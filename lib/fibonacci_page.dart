...
const String fibonacciDescription =
    'The Fibonacci sequence is the series of numbers where each number is the sum of the two preceding numbers, usually starting with 0 and 1.';
...
class _FibonacciPageState extends State<FibonacciPage>
  ...
  List<int> _generateFibonacciSequence(int n) {
    if (n <= 0) return [];
    if (n == 1) return [0];
    if (n == 2) return [0, 1];
  ...
  void _calculateFibonacci() {
    ...
    final sequence = _generateFibonacciSequence(input);
    setState(() {
      _sequence = sequence.join(', ');
      _result = 'The Fibonacci sequence up to $input terms is:';
      _error = null;
      _showResult = true;
    });
  }
